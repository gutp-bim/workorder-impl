...
#
#   IoTEvent → obs-collector (NATS) → obs-analyzer → Issue
#   → Ticket → Estimate承認 (NATS) → WO自動発行
...
#   NATS_PROPAGATION_WAIT_SEC=5 bash demo/e2e_iot_to_payment.sh
#
# 既知の仕様 vs 実装の差分:
#   - Booking競合: 仕様「CONFLICTED ステータスで生成・発行継続」
#                  実装「409 を返して Booking 拒否」(wo-manager/main.py:203)
...
# ── サービス URL ──────────────────────────────────────────────────────────────
OBS_COLLECTOR="${OBS_COLLECTOR_URL:-http://localhost:8001}"
ISSUE_MANAGER="${ISSUE_MANAGER_URL:-http://localhost:8002}"
TICKET_MANAGER="${TICKET_MANAGER_URL:-http://localhost:8003}"
WO_MANAGER="${WO_MANAGER_URL:-http://localhost:8004}"
PAYMENT_MANAGER="${PAYMENT_MANAGER_URL:-http://localhost:8005}"
OPS_DASHBOARD="${OPS_DASHBOARD_URL:-http://localhost:8006}"

# NATS メッセージ伝播と HTTP 非同期処理の待機秒数
NATS_WAIT="${NATS_PROPAGATION_WAIT_SEC:-3}"
...
# ── ヘルスチェック ────────────────────────────────────────────────────────────
step "0. サービス起動確認"
for svc_url_label in \
        "${OBS_COLLECTOR}/healthz:obs-collector" \
        "${ISSUE_MANAGER}/healthz:issue-manager" \
        "${TICKET_MANAGER}/healthz:ticket-manager" \
        "${WO_MANAGER}/healthz:wo-manager" \
        "${PAYMENT_MANAGER}/healthz:payment-manager" \
...
# ── Step 1: IoTEvent 投入 ─────────────────────────────────────────────────────
step "1. IoTEvent 投入 → obs-collector POST /ingest/iot-event"
IOT_RESP=$(curl_post "${OBS_COLLECTOR}/ingest/iot-event" '{
...
# ── Step 2: NATS 伝播待ち ─────────────────────────────────────────────────────
step "2. NATS 伝播待ち: obs.iot-event.created → obs-analyzer → issue-manager (${NATS_WAIT}s)"
sleep "${NATS_WAIT}"

# ── Step 3: Issue 自動生成確認 ───────────────────────────────────────────────
step "3. Issue 自動生成確認 → issue-manager GET /issues"
ISSUES=$(curl_get "${ISSUE_MANAGER}/issues")
ISSUE_COUNT=$(echo "${ISSUES}" | jq 'length')

if [ "${ISSUE_COUNT}" -eq 0 ]; then
    fail "Issue が生成されていません。obs-analyzer のログを確認してください: docker compose logs obs-analyzer"
fi

# 最新 Issue を取得（NATS受信順で最後尾）
...
# ── Step 4: Ticket 起票 ──────────────────────────────────────────────────────
step "4. Ticket 起票 → ticket-manager POST /tickets"
TICKET_RESP=$(curl_post "${TICKET_MANAGER}/tickets" "$(jq -n \
...
# ── Step 5: Estimate 作成 ────────────────────────────────────────────────────
step "5. Estimate 作成 → ticket-manager POST /estimates"
ESTIMATE_RESP=$(curl_post "${TICKET_MANAGER}/estimates" "$(jq -n \
...
# ── Step 6: Estimate 承認 ────────────────────────────────────────────────────
step "6. Estimate 承認 → ticket-manager PATCH /estimates/${ESTIMATE_ID}/approve"
APPROVE_RESP=$(curl_patch "${TICKET_MANAGER}/estimates/${ESTIMATE_ID}/approve")
APPROVED_STATUS=$(echo "${APPROVE_RESP}" | jq -r '.estimate_status')
ok "Estimate 承認: status=${APPROVED_STATUS}"

# ── Step 7: NATS 伝播待ち ────────────────────────────────────────────────────
step "7. NATS 伝播待ち: ticket.estimate.approved → wo-manager (${NATS_WAIT}s)"
sleep "${NATS_WAIT}"

# ── Step 8: WorkOrder 自動発行確認 ───────────────────────────────────────────
step "8. WorkOrder 自動発行確認 → wo-manager GET /work-orders"
WO_LIST=$(curl_get "${WO_MANAGER}/work-orders")
WO_COUNT=$(echo "${WO_LIST}" | jq 'length')

if [ "${WO_COUNT}" -eq 0 ]; then
    fail "WorkOrder が生成されていません。wo-manager のログを確認: docker compose logs wo-manager"
fi
...
step "9. ServiceTask 完了報告 → wo-manager"
if [ -z "${TASK_IDS}" ]; then
    ...
    for TASK_ID in ${TASK_IDS}; do
        COMPLETE_RESP=$(curl_patch "${WO_MANAGER}/work-orders/${WO_ID}/tasks/${TASK_ID}/complete")
...
# ── Step 10: Payment 登録 ────────────────────────────────────────────────────
step "10. Payment 登録 → payment-manager POST /payments"
PAYMENT_RESP=$(curl_post "${PAYMENT_MANAGER}/payments" "$(jq -n \
    --arg wo_id "${WO_ID}" \
    '{
...
echo "  通知配信ログ      : docker compose logs notify-dispatcher"
echo "  全フロー詳細      : curl -s ${OPS_DASHBOARD}/ops/flows | jq ."
echo ""
echo "仕様 vs 実装の既知差分:"
echo "  Booking競合 : 仕様=CONFLICTED ステータスで継続、実装=409 で拒否"
echo "                → services/wo_manager/gutp_wo_manager/main.py:203"
